...
router.get('/:username', verifyToken_1.default, async (req, res) => {
    ...
    if (cachedUser) {
        ...
        uid = await userRef.where("username", "==", req.params.username).get().then(async (data) => {
            ...
            await (0, redisConnect_1.SET_ASYNC)(`profile ${req.params.username}`, JSON.stringify(temp[0]), "EX", 1000);
            return temp[0];
        });
    }
    if (!uid) {
    ...
    await (0, redisConnect_1.SET_ASYNC)(`mpost ${req.params.username}`, JSON.stringify(mfeed), "EX", 1000).catch(err => console.log(err));
    res.json(mfeed);
    return;
});
// Get specific post 
router.get("/id/:postId", verifyToken_1.default, async (req, res) => {
    ...
    if (cachedPost) {
        ...
        await (0, redisConnect_1.SET_ASYNC)(req.params.postId, JSON.stringify(pst.data()), 'EX', 600).then(res.json(pst.data()));
    }
});
router.get("/del/:postId", verifyToken_1.default, async (req, res) => {
    ...
    if (postData.userId === req.user.id) {
        ...
        await (0, imageDelete_1.default)(req.params.postId);
        await (0, redisConnect_1.DEL_ASYNC)(`mpost ${req.user.username}`);
